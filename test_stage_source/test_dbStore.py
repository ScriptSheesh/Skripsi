...
def process_message(cursor, sender_number, message):
    ...
    if contains_url(message):
        ...
        if result:
            ...
            return f"Link {message} terakhir dilaporkan pada {timestamp} sebanyak: {count + 1}"
        else:
            # If the message does not exist, save it in the database
            save_message(cursor, sender_number, message)
            return "Link baru saja dilaporkan oleh anda"
    else:
        return "Silahkan input url yang valid"

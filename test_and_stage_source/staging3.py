...
def count_iframes(url):
    ...
    try:
        response = requests.get(url, headers=headers, timeout=10)  # Adding headers and timeout
        html_content = response.content
        soup = BeautifulSoup(html_content, 'html.parser')
        iframes = soup.find_all('iframe')
        return len(iframes)
    except RequestException as e:  # Catching more specific exceptions
...
def detect_url_redirect(url):
    try:
        ...
        print("Error:", e)
        return 0  # Not redirected or error occurred
...
def has_url_title(url):
    ...
    try:
        ...
        # Returns 1 if title exists and is not empty, 0 otherwise
        return 1 if title and title.text.strip() else 0
    except Exception:
        return 0  # In case of request failure or parsing error, return 0
...
def get_webpage_title(url):
    try:
        ...
        # Pastikan permintaan berhasil dengan kode status 200
        if response.status_code == 200:
            # Parsing konten HTML menggunakan BeautifulSoup
            soup = BeautifulSoup(response.content, 'html.parser')
            
            # Cari tag <title> dan ambil teksnya
            title_tag = soup.find('title')
            
            # Kembalikan teks judul jika tag <title> ditemukan
            if title_tag:
                return title_tag.text.strip()
            else:
                return "Title not found"  # Tag <title> tidak ditemukan
        else:
            return "Failed to retrieve content"  # Gagal mengambil konten dari URL
    except requests.exceptions.RequestException as e:
        # Tangani kesalahan permintaan (misalnya, timeout, gagal terhubung, dll.)
...
def title_match_scoring(webpage_title, url):
    def url_title_match_score(t_set, txt_url):
        ...
        for element in t_set:
            if txt_url.find(element.lower()) >= 0:  # Perbaikan: memastikan pencarian case-insensitive
                n = len(element)
                score += base_score * n
                txt_url = txt_url.replace(element.lower(), "", 1)  # Perbaikan: menghapus elemen yang cocok
            if score > 99.9:
                score = 100
                break
        return score

    t_set = webpage_title.lower().split()  # Memecah judul menjadi set kata dan memastikan lowercase

    # Ekstrak root domain dari URL
    parsed_url = urlparse(url)
    root_domain = parsed_url.netloc.lower()  # Memastikan lowercase
    root_domain = re.sub(r'^www\.', '', root_domain)  # Hapus www.
    root_domain = root_domain.split('.')[0]  # Ambil bagian sebelum TLD
...
def check_and_save_rulebased(url):
    ...
    if ssl_version == "Error: URL not found or SSL version could not be retrieved":
        # Immediately return if there's an error retrieving SSL version
        return "Error: SSL version could not be retrieved, possibly due to the URL being unreachable.", "N/A"

    try:
        # Continue with the rest of the analysis as SSL version was successfully retrieved
        tld = get_tld(url)
        ...
        # Calculate phishing chance based on the aggregated criteria
        criteria_met = sum([
            is_cyrillic,
            count_special_char,
        ...
        # Adjust the phishing chance calculation as per your criteria

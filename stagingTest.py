...
def detect_url_redirect(url):
    try:
        # Send a request to the URL without allowing redirects
        response = requests.get(url, allow_redirects=False)
        
        # Check for server-side redirection (HTTP status codes 301 and 302)
        if response.status_code == 301 or response.status_code == 302:
            return response.headers['Location']  # Return the redirected URL
        
        # Check for client-side redirection (JavaScript-based)
        else:
            html_code = response.text
            
            # Define regular expressions to search for URL redirection patterns in HTML content
            patterns = [
...
# Function to check if URL is using HTTPS
def is_https(url):
    try:
        response = requests.get(url)
...
# Process 2 URLs at Once with Comma Value V1.0
# def process_message(cursor, sender_number, message):
#     if contains_url(message):
...
#             rule_based_response, phishing_chance = check_and_save_rulebased(url)
...
# Process 2 URLs at Once with Comma Value V2.0
def process_message(cursor, sender_number, message):
    if contains_url(message):
        urls = [url.strip() for url in message.split(',')]
        responses = []
        ...
        for url in urls:
            ...
            if result:
                ...
                phishing_chance = result[11]  # Assuming the phishing chance is stored in the third column
                
                response_message = f"\nURL {rule_based_response} telah dilaporkan sebelumnya dan memiliki kemungkinan sebagai URL phishing sebesar 🚩: {phishing_chance}"
            else:
                phishing_urls = load_phishing_urls()
...
# # Process 2 URLs at Once with Enter Value 
# def process_message(cursor, sender_number, message):
#     if contains_url(message):
...
#             # Check if the input URL matches any variation in the phishing database
#             if url in phishing_urls:
#                 list_based_response = f"URL {url} terdeteksi merupakan phishing pada database kami.\n"
#             else:
#                 # Check if the input URL with a trailing slash matches any variation in the phishing database
#                 message_with_slash = url + '/'
#                 if message_with_slash in phishing_urls:
...
#                         cursor.execute("UPDATE messages SET timestamp=CURRENT_TIMESTAMP, count=? WHERE message_body=?", (count + 1, url))
#                         cursor.connection.commit()
#                         list_based_response = f"URL {url} tidak ditemukan dalam database phishing kami.\nTerakhir dilaporkan pada {timestamp} sebanyak {count} kali.\n"
#                     else:
#                         list_based_response = "0"
...
# Debuging URL Redirect 
# def process_message(cursor, sender_number, message):
#     if contains_url(message):
#         redirected_url = detect_url_redirect(message)
#         if redirected_url:
#             return f"URL terdeteksi dialihkan dengan kode status: {redirected_url}"
...
def check_and_save_rulebased(url):
    try:
        ...
        phishing_chance = "100%" if criteria_met == 9 else "44%" if criteria_met >= 4 else "22%" if criteria_met >= 2 else "11%" if criteria_met >= 1 else "URL tidak terdeteksi phishing"

        db = get_db()
        cursor = db.cursor()
        cursor.execute("INSERT INTO rulebased (url, tld, ssl_ver, is_cyrillic, has_submit_button, has_password_field, iframe_count, is_obfuscated, redirected_url, is_https, phishing_chance) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)", 
            (url, 'Common' if is_common_tld else 'Uncommon', ssl_version, 1 if is_cyrillic else 0, 1 if has_submit_button else 0, 1 if has_password_field else 0, iframe_count, 1 if is_obfuscated else 0, '301' if redirected_url else '302', 1 if is_https else 0, phishing_chance))
        db.commit()
